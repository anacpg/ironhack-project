...
/**
 * Get hash (#) elements from url and returns a map where:
 * - key = unescaped hash element name
 * - value =  unescaped hash element value
 */
    ...
    if (urlSplitted.length == 2) {
        ...
        for (var i=0; i<arrElements.length; i++) {
            ...
            if (keyAndValue.length == 2) {
                //there is an element that has the following format: #element=[value] (value is optional)
...
function WaitDialog(language, imageUrl) {
    var translations = { "en" : { "loading" : "Loading..." },
            "es" : { "loading" : "Cargando..." },
            "ca" : { "loading" : "Buscando..." },
            "fr" : { "loading" : "Recherche..." },
            "de" : { "loading" : "Suchen..." },
            "ja" : { "loading" : "\u691C\u7D22\u4E2D\u002E\u002E\u002E" },
            "pt" : { "loading" : "Carregando..." },
            "it" : { "loading" : "Caricando..." },
            "ru" : { "loading" : "Загружается..." },
            "pl" : { "loading" : "Trwa ładowanie..." },
            "zh" : { "loading" : "加载"},
            "sv" : { "loading" : "Cargando..." },
            "da" : { "loading" : "Cargando..." },
            "no" : { "loading" : "Cargando..." },
    ...
    if (language==null || language=='' || typeof translations[language] == "undefined") {
        language = "en";
    }
    ...
    if (mjQuery('#mirai_overlay, #mirai_loadingBox').length == 0) {
        ...
        if (imageUrl) {
            htmlMiraiLoadingBox += "<img src=\"" + imageUrl + "\" /><p></p></div>";
        } else {
        ...
        if (!mjQuery.support.style && !mjQuery.support.opacity && !(typeof document.documentElement.style.maxHeight != "undefined"))
...
WaitDialog.prototype.show = function() {
    if (mjQuery('#mirai_overlay, #mirai_loadingBox').length > 0) {
        mjQuery('#mirai_overlay').css('opacity', '0.8').animate({ opacity:0.4 }, 500);
        mjQuery('#mirai_loadingBox').css('opacity', '1').animate({ opacity:1 }, 300);
...
WaitDialog.prototype.hide = function() {
    if (mjQuery('#mirai_overlay, #mirai_loadingBox').length > 0 && mjQuery('#mirai_overlay, #mirai_loadingBox').is(":visible")) {
...
function convertPrices(valueStr, defaultCurrencyInfo, currentCurrencyInfo, removeCurrencySymbol, allowNegativeValues) {
 if (!valueStr || (valueStr.charAt(0) === "-" && !allowNegativeValues)) {
     valueStr = "0.00";
 }
 var priceInCents = extractPriceInCents(valueStr, defaultCurrencyInfo.numDecimals);
 var cartPriceInCentsNewCurrency = priceInCents * currentCurrencyInfo.value * Math.pow(10, currentCurrencyInfo.numDecimals) / (defaultCurrencyInfo.value * Math.pow(10, defaultCurrencyInfo.numDecimals));
 var formattedPrice = convertPriceInCentsToString(cartPriceInCentsNewCurrency, currentCurrencyInfo.numDecimals, currentCurrencyInfo.formatWithSymbol);
...
function extractPriceInCents(text, numDecimals) {
 return text.match(/\.*(-?\d+[\.,]?\d*).*/)[1].replace(",",".") * Math.pow(10, numDecimals);
}

function convertPriceInCentsToString(priceInCents, currencyNumDecimals, currencyFormat) {
 var priceValue = convertPriceInCentsToStringWithoutCurrency(priceInCents, currencyNumDecimals, currencyFormat);
 return priceValue.replace(/(\d+)[\.]?(\d*)/, currencyFormat);
}

function convertPriceInCentsToStringWithoutCurrency(priceInCents, currencyNumDecimals, currencyFormat) {
 return parseFloat(priceInCents / Math.pow(10, currencyNumDecimals)).toFixed(currencyNumDecimals);
}

function styleHtmlForPrice(priceFormatted, currencyFormat, removeCurrencySymbol, addArrowSymbol) {
 var htmlStyle = "";
 if (priceFormatted.match(/[^\d]*(\d*)[.,](\d*).*/)) {
     htmlStyle = priceFormatted.replace(/[^\d]*(\d*)[.,](\d*).*/, currencyFormat);
...
//NEW
var I18n = (function () {
 function _getMessage(messages, key, arguments) {
     var message = messages[key] || key;
     var argumentsSplitted = arguments ? arguments.split(",") : [];
     for (var i = 0; i < argumentsSplitted.length; i++) {
         var replace = "\\{" + (i) + "\\}";
         message = message.replace(new RegExp(replace), argumentsSplitted[i]);
     }
     return message;
 }
 return {
...
function getCurrentCurrencyMirai() {
 var currentCurrencyCode = mjQuery("#currencyDropdownTop > a > span").attr("class");
...
function putFlagImageToCurrency(currencyCode) {
 mjQuery("#currencyDropdownTop > a").css('background-image', 'url(' + bookingEntrance.params.protocol + '//' + bookingEntrance.params.hostsInfo.cdn + '/images/flags/' + currencyCode.toLowerCase() + '.gif )');
}
...
var bookingEntrance = {
    ...
    selectTranslations : function(language) {
        ...
        var translations = {
            "en" : { "checkin" : "Check-in" , "nights" : "Nights", "departure" : "(departure *)", "departureDateFormat" : "d M", "clientcode" : "Promotional code", "startbooking" : "Check Rates and Book", "track" : "Track/Cancel Reservation", "loading" : "Searching...", "ajaxTimeoutMsg" : "Temporary problem. Try again later", "chainSelectEmpty" : "Search by location or hotel"},
            "es" : { "checkin" : "Entrada" , "nights" : "Noches", "departure" : "(salida *)", "departureDateFormat" : "d M", "clientcode" : "C\u00F3digo promocional", "startbooking" : "Ver tarifas y reservar", "track" : "Ver/Cancelar reserva", "loading" : "Buscando...", "ajaxTimeoutMsg" : "Problema t\u00E9cnico moment\u00E1neo. Vuelve a probar m\u00E1s tarde", "chainSelectEmpty" : "Busca en una localización o en un hotel"},
            "ca" : { "checkin" : "Arribada" , "nights" : "Nits", "departure" : "(sortida *)", "departureDateFormat" : "d M", "clientcode" : "Codi promocional", "startbooking" : "Veure tarifes i reservar", "track" : "Veure/Cancel\u00B7lar reserva", "loading" : "Buscando...", "ajaxTimeoutMsg" : "Problema t\u00E8cnic momentani. Torni a provar m\u00E9s tard", "chainSelectEmpty" : "Search by location or hotel"},
            "de" : { "checkin" : "Ankunft" , "nights" : "N\u00E4chte", "departure" : "(Abreise *)", "departureDateFormat" : "d. m.", "clientcode" : "Promo Code", "startbooking" : "Zur Buchung", "track" : "Buchung ansehen/stornieren", "loading" : "Suchen...", "ajaxTimeoutMsg" : "Vor\u00FCbergehendes Problem. Versuchen Sie es sp\u00E4ter noch einmal", "chainSelectEmpty" : "Search by location or hotel"},
            "fr" : { "checkin" : "Arriv\u00E9e" , "nights" : "Nuits", "departure" : "(départ *)", "departureDateFormat" : "d M", "clientcode" : "Code promo", "startbooking" : "Voir les tarifs et r\u00E9server", "track" : "Voir/annuler r\u00E9servation", "loading" : "Recherche...", "ajaxTimeoutMsg" : "Probl\u00E8me technique temporaire. Merci d'essayer \u00E0 nouveau un peu plus tard", "chainSelectEmpty" : "Search by location or hotel"},
            "ja" : { "checkin" : "\u30C1\u30A7\u30C3\u30AF\u30A4\u30F3" , "nights" : "\u5BBF\u6CCA\u6570", "departure" : "(出発日 *)", "departureDateFormat" : "m月d日", "clientcode" : "\u30D7\u30ED\u30E2\u30B3\u30FC\u30C9", "startbooking" : " \u6599\u91D1\u3092\u898B\u308B\u30FB\u4E88\u7D04\u3059\u308B", "track" : "\u4E88\u7D04\u3092\u898B\u308B\u30FB\u30AD\u30E3\u30F3\u30BB\u30EB\u3059\u308B", "loading" : "\u691C\u7D22\u4E2D\u002E\u002E\u002E", "ajaxTimeoutMsg" : "\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u3057\u3070\u3089\u304F\u3057\u3066\u304B\u3089\u3082\u3046\u4E00\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002", "chainSelectEmpty" : "Search by location or hotel"},
            "it" : { "checkin" : "Arrivo" , "nights" : "Notti", "departure" : "(partenza *)", "departureDateFormat" : "d M", "clientcode" : "Codice promozionale", "startbooking" : "Vedere prezzi e prenotare", "track" : "vedere/Cancellare prenotazione ", "loading" : "Cercando...", "ajaxTimeoutMsg" : "Problema momentaneo provare piu' tardi", "chainSelectEmpty" : "Search by location or hotel"},
            "pt" : { "checkin" : "Check-in" , "nights" : "Noites", "departure" : "(saída *)", "departureDateFormat" : "d M", "clientcode" : "C\u00F3digo promocional", "startbooking" : "Ver tarifas e reservar", "track" : "Seguir/Cancelar Reserva", "loading" : "Buscando...", "ajaxTimeoutMsg" : "Problema t\u00E9cnico. Por favor tente mais tarde", "chainSelectEmpty" : "Busca por localiza\u00E7\u00E3o ou hotel"},
            "ru" : { "checkin" : "Дата заезда" , "nights" : "Кол-во ночей", "departure" : "(отъезд *)", "departureDateFormat" : "d M", "clientcode" : "Промо-код", "startbooking" : "Искать", "track" : "Проверить/Аннулировать бронирование", "loading" : "Загрузка...", "ajaxTimeoutMsg" : "Временно недоступно. Попробуйте еще раз", "chainSelectEmpty" : "Search by location or hotel"},
            "pl" : { "checkin" : "Check-in" , "nights" : "Nights", "departure" : "(wyjazd *)", "departureDateFormat" : "d M", "clientcode" : "Promotional code", "startbooking" : "Check Rates and Book", "track" : "Track/Cancel Reservation", "loading" : "Searching...", "ajaxTimeoutMsg" : "Temporary problem. Try again later", "chainSelectEmpty" : "Search by location or hotel"},
            "zh" : { "checkin" : "入住" , "nights" : "晚", "departure" : "(退房 *)", "departureDateFormat" : "d M", "clientcode" : "促销代码", "startbooking" : "查询价格与预定", "track" : "查阅/确认预定", "loading" : "查询中...", "ajaxTimeoutMsg" : "占时无法执行. 请稍后再试", "chainSelectEmpty" : "由地址或酒店名称寻找"},
            "sv" : { "checkin" : "Ankomst" , "nights" : "Nätter", "departure" : "(avresa *)", "departureDateFormat" : "d M", "clientcode" : "Kampanjkod", "startbooking" : "Se priser och boka", "track" : "Se/Avbryt bokning", "loading" : "Söker…", "ajaxTimeoutMsg" : "Tillfälligt tekniskt problem. Försök igen senare.", "chainSelectEmpty" : "Sök på en plats eller ett hotell"},
            "da" : { "checkin" : "Ankomst" , "nights" : "Nætter", "departure" : "(afrejse *)", "departureDateFormat" : "d M", "clientcode" : "Kampagnekode", "startbooking" : "Se priser og reservér", "track" : "Se/annuller reservation", "loading" : "Søger…", "ajaxTimeoutMsg" : "Kortvarigt teknisk problem. Forsøg igen senere", "chainSelectEmpty" : "Søg efter steder eller efter et bestemt hotel"},
            "no" : { "checkin" : "Innsjekking" , "nights" : "Netter", "departure" : "(utsjekking *)", "departureDateFormat" : "d M", "clientcode" : "Lanseringskode", "startbooking" : "Se priser og bestillinger", "track" : "Se/anullere bestillingen", "loading" : "Søker...", "ajaxTimeoutMsg" : "Forbigående teknisk svikt. Prøv igjen senere", "chainSelectEmpty" : "Søk på et sted eller et hotell"},
            "fi" : { "checkin" : "Saapuminen" , "nights" : "Yötä", "departure" : "(lähtö*)", "departureDateFormat" : "d M", "clientcode" : "Kampanjakoodi", "startbooking" : "Katso hinnat ja varaa", "track" : "Katso/peruuta varaus", "loading" : "Etsii…", "ajaxTimeoutMsg" : "Väliaikainen tekninen ongelma. Yritä myöhemmin uudelleen", "chainSelectEmpty" : "Etsi tietyn paikan tai hotellin perusteella"},
    ...
    loadLayout : function() {
        if (bookingEntrance.params.doctype=='html4') {
            bookingEntrance._addHtml4();
        } else {
            bookingEntrance._addXhtml();
        }
        ...
        if (chainOption.length == 2) {
            ...
            if (chainOption[0] == "location") {
                var params = {
                    ...
                    'sort'         : "hotelname",
                    'bookinghost'  : bookingEntrance.params.bookinghost ? bookingEntrance.params.bookinghost : "",
                    'bookingpage'  : bookingEntrance.params.static_page ? bookingEntrance.params.static_page : "",
                    'securepage'   : bookingEntrance.securepage ? bookingEntrance.securepage : "",
                    'idoffers'     : bookingEntrance.idoffers
                };
                document.location.href = bookingEntrance.params.protocol + "//" + bookingEntrance.params.hostsInfo.book + "/XMAS/reservation/forwarder.htm?" + mjQuery.param(params);
            } else {//user has selected an hotel from chain
                mjQuery('input[name=idtokenprovider]').val(chainOption[1]);
    ...
    _redirectToForwarded : function(forwarderRequest) {
        ...
        var urlForwarder = bookingEntrance.params.protocol + "//" + bookingEntrance.params.hostsInfo.book + "/XMAS/reservation/" + forwarderRequest;
...
//        mjQuery.cookie(bookingEntrance.cookieName, JSON.stringify(bookingCookie), { expires : 7, path: '/', domain:  bookingEntrance.getDomain(window.location.hostname)});
        ...
        mjQuery('#' + bookingEntrance.divId).html(
                '<form id="formAvailability" action="" ' +
                  ...
                  '<div id="mirai_be0">' +
                    ...
                    (bookingEntrance.params.idchain ? bookingEntrance._getSelectForChain() : "") +
    ...
    _getSelectForChain : function() {
        var html = '<select id="mirai_be9" name ="chainselector">';
